...
import { NotificationAttributes, NotificationInstance } from '@database/models/notification'
...
class NotificationService {
  ...
  public static async findAll(req: Request): Promise<DtoPaginate> {
    const { includeCount, order, ...queryFind } = PluginSqlizeQuery.generate(req.query, Notification, [])
  ...
  public static async findByPk(
    ...
    if (!data) {
      throw new ResponseError.NotFound('notification data not found or has been deleted')
    }
   ...
   * @returns
   */
  public static async findById(id: string, paranoid?: boolean): Promise<NotificationInstance> {
   ...
   * @returns
   */
  public static async create(formData: NotificationAttributes, txn?: Transaction): Promise<NotificationInstance> {
   ...
   * @returns
   */
  public static async update(id: string, formData: Partial<NotificationAttributes>, txn?: Transaction): Promise<NotificationInstance> {
   ...
   * @param force
   */
  public static async multipleDelete(ids: string[], force?: boolean): Promise<void> {
    const isForce = validateBoolean(force)
  ...
  public static async sendNotification(): Promise<void> {
    const getNotification = await Notification.findOne({
      where: {
        ...
        isSend: false,
        sendAt: {
          [Op.and]: [{ [Op.gte]: startOfDay(new Date()) }, { [Op.lt]: new Date() }],
        },
      },
    })
    ...
    if (getNotification) {
      ...
      if (_.isEmpty(getNotification.UserId)) {
        ...
        const getFcmToken = await FcmTokenService.findByUser(String(getNotification.UserId))
        ...
        if (!_.isEmpty(getFcmToken.token)) {
          ...
          console.log(chalk.cyan('sending to user notification...'), dataFCM.results)
        }
      }
    }
  }
