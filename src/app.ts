...
class App {
  ...
  private plugins(): void {
    ...
    this.application.use(function (req: Request, res: Response, next: NextFunction) {
      new withState(req)
      next()
    })
  }
  ...
  private docsSwagger(): void {
    ...
    this.application.get('/v1/api-docs', swaggerUI.setup(swaggerSpec, optionsSwaggerUI))
  }
  ...
  public run(): void {
    ...
    this.application.use(async function handleRollbackTransaction(err: any, req: Request, res: Response, next: NextFunction): Promise<void> {
      try {
